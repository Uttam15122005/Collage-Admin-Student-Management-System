...
function wpsp_UpdateStudent()
{
	...
	if (isset($_POST['Class']) && isset($_POST['s_rollno'])) {
		global $wpdb;
		$classIDs = array_map('intval', $_POST['Class']);
		$roll_number = sanitize_text_field($_POST['s_rollno']);
		$wp_user_id = intval($_POST['wp_usr_id']);

		foreach ($classIDs as $classID) {
			$students = $wpdb->get_results(
				$wpdb->prepare(
					"SELECT class_id, s_rollno FROM $wpsp_student_table WHERE s_rollno = %s AND wp_usr_id != %d",
					$roll_number,
					$wp_user_id
				)
			);
			...
			foreach ($students as $student) {
				$stored_class_ids = maybe_unserialize($student->class_id);

				// Ensure $stored_class_ids is an array
				if (is_array($stored_class_ids) && in_array($classID, $stored_class_ids)) {
					echo esc_html("The roll number $roll_number is already assigned to another student in class ID: $classID.", "wpschoolpress");
					exit;
				}
			}
		}
